...
class TrainerWithFreezer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    freezer: _Freezer | None # : Freezer for timelyfreeze
    ...
    def __init__(self, job_config: TimelyFreezeConfig):
        ...
        #  - setting freezer-related configs
        job_config.initialize(self)  #  - this code requires trainer.model_parts, trainer.pp_schedule and trainer.parallel_dims
    ...
    def train_step(
        ...
        if self.job_config.parallelism.pp > 1:
            pplog.pipeline_log.enable()  # : enable PP logging for each train step
        ...
        if self.job_config.parallelism.pp > 1:
            pplog.pipeline_log.disable()  # : disable PP logging after each train step
