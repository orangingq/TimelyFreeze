...
class _PipelineStageBase(ABC):
    ...
    def forward_one_chunk(
        ...
        """
        ######### Addition #########
        from timelyfreeze.core import logger as pplog #  - to log the forward GPU time
        ####### Addition End #######
        ...
        try:
            ######### Addition #########
            with pplog.pipeline_log.forward(microbatch=fwd_chunk_id, stage=self.stage_index): #  - to log the forward GPU time
            ####### Addition End #######
    ...
    def backward_one_chunk(
        ...
        if self.dw_builder:
            ...
            if full_backward:
                ########## Addition ##########
                with pplog.pipeline_log.backward(microbatch=bwd_chunk_id, stage=self.stage_index) as freeze_list: #  - to log the backward GPU time
                    bwd_kwargs["freeze_list"] = freeze_list #  - to set freeze list locally
                ######## Addition End ########
                    self.dw_builder()()
            else:
                ########## Addition ##########
                with pplog.pipeline_log.backward_weight(microbatch=bwd_chunk_id, stage=self.stage_index) as freeze_list: #  - to log the backward GPU time
                    bwd_kwargs["freeze_list"] = freeze_list #  - to set freeze list locally
                ######## Addition End ########
            ...
            if full_backward:
                ...
                if not self.is_first:
                    ...
                    ########## Addition ##########
                    with pplog.pipeline_log.backward_input(microbatch=bwd_chunk_id, stage=self.stage_index) as freeze_list: #  - to log the backward GPU time
                        bwd_kwargs["freeze_list"] = freeze_list #  - to set freeze list locally
                    ######## Addition End ########
    ...
    def backward_weight_one_chunk(self, bwd_chunk_id: int, last_backward=False):
        ...
        from timelyfreeze.core import logger as pplog #  - to log the backward GPU time
