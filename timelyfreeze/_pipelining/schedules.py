...
class ScheduleGPipe(PipelineScheduleSingle):
    ...
    def _step_microbatches(
        ...
        ##########   Addition ##########
        bwd_recvs_to_wait: list[list[dist.Work]] = []
        ops = self._stage.get_bwd_recv_ops(0)
        works = _sorted_batch_p2p(ops, desc="bwd_recv")
        bwd_recvs_to_wait.append(works.values())
        ########   Addition End ########
        ...
        for i in range(self._n_microbatches):
            with record_function(f"Backward {i}"):
                ...
                ############ Modification ############# 
                if i < self._n_microbatches-1:
                    ops = self._stage.get_bwd_recv_ops(i+1)
                    works = _sorted_batch_p2p(ops, desc="bwd_recv")
                    bwd_recvs_to_wait.append(works.values())

                for work in bwd_recvs_to_wait[i]:
                    _wait_batch_p2p(work)
                ########### Modification End ##########
...
class Schedule1F1B(PipelineScheduleSingle):
    ...
    def _step_microbatches(
        ...
        #########   Addition ############
        bwd_recvs_to_wait: list[list[dist.Work]] = [None] * self._n_microbatches
        #########   Addition End ########
        # 1B1F phase
        while True:  # Don't worry, we have a break inside
            ...
            ############ Modification #############  
            if bwd_mb_index == 0:
                bwd_recvs_to_wait[0] = self._stage.get_bwd_recv_ops(0)
            if bwd_mb_index < self._n_microbatches-1:
                bwd_recvs_to_wait[bwd_mb_index+1] = self._stage.get_bwd_recv_ops(bwd_mb_index+1)
            bwd_recvs = bwd_recvs_to_wait[bwd_mb_index]
            ########### Modification End ##########
...
class PipelineScheduleMulti(_PipelineSchedule):
    ...
    def __init__(
        ...
        if stage_index_to_group_rank is not None:
            ...
            for i, stage in enumerate(self._stages):
                stage.stage_index_to_group_rank = stage_index_to_group_rank[i]
    ...
    def _step_microbatches(
        ...
        for stage_index, stage in stage_index_to_stage.items():
            # TODO: assumption that stages only communicate from distances of +1/-1 (no skip connections)
            if stage_index > 0:
                all_prev_ranks.add(stage.stage_index_to_group_rank[stage_index - 1])
            if stage_index < self._num_stages - 1:
                all_next_ranks.add(stage.stage_index_to_group_rank[stage_index + 1])
