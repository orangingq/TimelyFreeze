...
def clip_grad_norm_(
    ...
    if pp_mesh is not None:
        if math.isinf(norm_type):
            ...
            dist.all_reduce(total_norm.to(pp_mesh.device_type), op=dist.ReduceOp.MAX, group=pp_mesh.get_group()) # 
        else:
            total_norm **= norm_type
            # dist.all_reduce(total_norm, op=dist.ReduceOp.SUM, group=pp_mesh.get_group())
            dist.all_reduce(total_norm.to(pp_mesh.device_type), op=dist.ReduceOp.SUM, group=pp_mesh.get_group()) # 
